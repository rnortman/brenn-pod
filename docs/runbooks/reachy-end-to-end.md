...
| `continuation_timeout_ms` | `[brenn]` | 30000 | How long a turn waits for a promised `<continued/>` segment, and so how long the head stays up mid-answer. Nothing is playing during the wait and the pipeline dispatches turns inline, so a wake word said inside it is not answered until it ends |
| `library_names` | `[brenn]` | unset | A copy of the deployed library's `library.names.json` (from `brenn-reachy/cogs/`, committed there), relative to `host/`. Names the poses and motions a reply may cue and the pace of each; the help document states the list verbatim. Unset means no cue vocabulary — every `<pose/>` and `<motion/>` in a reply is stripped. Copy it again after every library regeneration: a name the unit no longer holds is refused per script by the daemon, with a line on both sides |
| `mode` | `[barge]` | `wake` | What may cut an audible reply. `wake`: only the wake word — the one interruption this machine can tell from its own voice. `speech`: sustained confident speech as well, which on this unit fires on the reply leaking back through the microphone. `mute`: nothing cuts, and nothing said over the reply is heard at all — the microphone is scored as silence while the pod is playing and for 400 ms after. The mode to run when echo cancellation cannot be trusted, at the price of a reply no voice can interrupt, the wake word included; a person who talks across the robot has to say it again |

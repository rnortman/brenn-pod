...
//! `PlaybackSink`.
//!
//! Platform-independent: the socket arrives as `&mut dyn std::io::Read` and the
//! playout device as `&mut dyn PlaybackSink`, so the same reassembly and handshake
//! rules serve every pod. Platform-specific instrumentation (heap waypoints on the
//! ESP32, plain logging elsewhere) enters through [`InboundObserver`].
...
use crate::wire::{DEVICE_PLAYBACK_FORMAT, PlaybackFormat};

/// The sample points inside the post-Hello inbound-audio window. Closed set: the
/// observer's log vocabulary is what operators grep, so a new site is a deliberate
/// addition here rather than an ad-hoc string at a call site.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum InboundWaypoint {
    /// Frame-cadence sample: the first accepted frame after Hello, then every tenth.
    Periodic,
    /// Socket-teardown sample, taken only on a connection that got past Hello.
    Exit,
}

impl InboundWaypoint {
    /// The site's log token, the trailing word in `heap wp inbound <token>`.
    pub const fn as_str(self) -> &'static str {
        match self {
            InboundWaypoint::Periodic => "periodic",
            InboundWaypoint::Exit => "exit",
        }
    }
}

/// Platform hook for the inbound path's observability points. The ESP pod samples heap
/// headroom here; hosts without a constrained heap install [`NoInboundObs`].
///
/// No default method bodies: an implementor that silently inherits a no-op would lose
/// the reading the hook exists to take.
pub trait InboundObserver {
    /// A post-Hello inbound-audio-window sample point. `frame` is the connection's
    /// accepted-frame count.
    fn waypoint(&mut self, site: InboundWaypoint, frame: u32);
    /// The inbound stream's declared format was accepted.
    fn hello_ok(&mut self, format: PlaybackFormat);
}

/// [`InboundObserver`] that records nothing — for hosts with no heap-pressure question
/// to answer, and for tests.
pub struct NoInboundObs;

impl InboundObserver for NoInboundObs {
    fn waypoint(&mut self, _site: InboundWaypoint, _frame: u32) {}
    fn hello_ok(&mut self, _format: PlaybackFormat) {}
}
...
///
/// `end_of_audio`, `flush_playback` and `stream_reset` are explicit forwards even
/// though the flood profile sends none of them: the trait's default no-op bodies
/// would silently swallow a control frame or connection boundary `inner`
/// (`I2sStreamSink`) needs to see, a latent trap for future reuse. `stream_reset`
/// is the worst to lose — nothing reports it, and the previous connection's banked
/// audio plays out under the new one.
pub struct StallCountingSink<'a> {
...
impl<'a> StallCountingSink<'a> {
    pub fn new(inner: &'a mut dyn PlaybackSink) -> Self {
...
impl FakeDacSink {
    ...
    fn accept_at(&mut self, pcm: &[u8], now: std::time::Instant) -> Accepted {
        if !is_valid_s16le_pcm(pcm) {
            // Invalid PCM is discarded: it neither fills the buffer nor counts as a
            // consumed frame.
            return Accepted::Enqueued;
        }
...
impl FrameAccumulator {
    ...
    pub fn has_complete_frame_held(&self) -> bool {
        if self.valid < 2 {
            return false;
        }
...
pub struct InboundConnectionState {
    seen_hello: bool,
...
impl Default for InboundConnectionState {
    fn default() -> Self {
        Self::new()
    }
}
...
impl InboundConnectionState {
    ...
    /// True once an inbound Hello with a matching protocol version has been decoded on
    /// this connection — including one whose declared *format* was then rejected, since
    /// the flag is set before the format check. Stays true until [`reset()`](Self::reset).
    ///
    /// Callers that instrument a teardown path use it to mute the faults that dropped
    /// the connection before any Hello was decoded (audio-before-Hello, version
    /// mismatch), which never entered the post-Hello audio window.
    pub fn seen_hello(&self) -> bool {
        self.seen_hello
    }

    /// Inbound audio frames accepted on this connection.
    pub fn inbound_frames(&self) -> u32 {
        self.inbound_frames
    }
}
...
///
/// Public (not private): callers that handle a socket EOF may still have frames
/// buffered in `accum` (from backpressure) that need a final drain.
pub fn consume_frames(
    accum: &mut FrameAccumulator,
    sink: &mut dyn PlaybackSink,
    state: &mut InboundConnectionState,
    obs: &mut dyn InboundObserver,
) -> std::io::Result<u32> {
    use crate::wire::{
        AUDIO_PROTOCOL_VERSION, DecodeError, FormatCheck, InboundFrame, MAX_FRAME_BYTES,
        check_inbound_format,
    };
    ...
    loop {
        ...
        match crate::wire::decode_inbound(&accum.buf[..frame_len]) {
            Ok(InboundFrame::Audio { pcm, .. }) => {
                ...
                match sink.accept(pcm) {
                    ...
                    Accepted::Enqueued => {
                        ...
                        // Sample the post-Hello inbound audio window: at the first frame
                        // after Hello-ok and every ~10 thereafter. The periodic streamer
                        // waypoints do not reach this window.
                        if state.inbound_frames == 1 || state.inbound_frames.is_multiple_of(10) {
                            obs.waypoint(InboundWaypoint::Periodic, state.inbound_frames);
                        }
                    }
                }
            }
            Ok(InboundFrame::Hello {
                ...
                match check_inbound_format(DEVICE_PLAYBACK_FORMAT, declared) {
                    FormatCheck::Match => obs.hello_ok(declared),
            ...
            Ok(InboundFrame::EndOfAudio) => {
                ...
                // Audio: without a validated format the stream is not trusted.
                if !state.seen_hello {
                    accum.reset();
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
...
pub fn drain_inbound(
    ...
    let frames_routed = consume_frames(accum, sink, state, obs)?;
    Ok(DrainOutcome {
        bytes_read,
        frames_routed,
    })
...
pub fn pump_inbound(
    ...
    for _ in 0..max_steps {
        if !drain_inbound(stream, accum, sink, state, obs)?.made_progress() {
...
/// Take the socket-teardown sample on a connection that got past Hello.
/// No-op if the connection never completed a Hello.
///
/// A Hello whose declared format was rejected counts as past-Hello — see
/// [`InboundConnectionState::seen_hello`].
pub fn note_inbound_exit(obs: &mut dyn InboundObserver, state: &InboundConnectionState) {
    if state.seen_hello {
        obs.waypoint(InboundWaypoint::Exit, state.inbound_frames);
    }
}
...
pub fn inbound_has_room(accum: &FrameAccumulator) -> bool {
...
mod tests {
    ...
    use super::{
        Accepted, FrameAccumulator, InboundConnectionState, InboundObserver, InboundWaypoint,
        NoInboundObs, PlaybackSink, StallCountingSink, consume_frames, drain_inbound,
        inbound_has_room, note_inbound_exit, pump_inbound,
    };
    ...
    /// `end_of_audio`, `flush_playback` and `stream_reset` are one-line forwards, not
    /// swallowed by the counting wrapper's own default no-op trait bodies.
    ///
    /// `stream_reset` matters most: a swallowed connection boundary lets the previous
    /// connection's banked audio play out under the new one, with nothing reporting it.
    #[test]
    fn stall_counting_sink_forwards_control_frames() {
        ...
        assert_eq!(
            inner.reset_calls, 1,
            "the connection boundary must reach inner's ring"
        );
    }
    ...
    fn encode(frame: &StreamFrame) -> Vec<u8> {
        ...
        let n = crate::wire::encode_frame(frame, &mut buf).expect("encode_frame in test helper");
        buf[..n].to_vec()
    }
    ...
    fn drain_complete_frame_one_read() {
        let mut pcm: heapless::Vec<u8, { crate::wire::MAX_AUDIO_PAYLOAD }> = heapless::Vec::new();
    ...
    fn drain_non_audio_variant_ignored() {
        let variants = [
            ...
            StreamFrame::Telemetry(crate::wire::Telemetry {
                device_ts_us: 1_000_000,
                kind: crate::wire::TelemetryKind::SpEnergy {
                    values: [1.0, 2.0, 3.0, 4.0],
                },
            }),
            StreamFrame::SegmentEnd(crate::wire::SegmentEnd {
                segment_id: 1,
                device_ts_us: 5_000_000,
                frames_sent: 10,
                samples_sent: 3200,
                reason: crate::wire::EndReason::VadRelease,
            }),
        ];
    ...
    fn inbound_hello(
        ...
        StreamFrame::Hello(crate::wire::Hello {
            version: crate::wire::AUDIO_PROTOCOL_VERSION,
            pod_id: heapless::String::try_from("pod-aabbcc").unwrap(),
            sample_rate_hz,
            bits_per_sample,
            channels,
            codec,
            channel_source: crate::wire::ChannelSource::AsrBeam,
        })
    }
    ...
    fn consume_with_state(
        ...
        consume_frames(accum, sink, state, &mut NoInboundObs)
    }

    /// A matching Hello is consumed (not routed to sink) and sets `seen_hello`.
    #[test]
    fn consume_frames_hello_match_ok() {
        let encoded = encode(&inbound_hello(16_000, 16, 1, crate::wire::Codec::S16Le));
        let mut accum = FrameAccumulator::new();
        let mut sink = CapturingSink::new();
        let mut state = InboundConnectionState::new();
        feed(&mut accum, &encoded);
    ...
    fn consume_frames_hello_mismatch_drops() {
        let encoded = encode(&inbound_hello(48_000, 16, 1, crate::wire::Codec::S16Le));
        let mut accum = FrameAccumulator::new();
        let mut sink = CapturingSink::new();
        let mut state = InboundConnectionState::new();
        feed(&mut accum, &encoded);
    ...
    fn consume_frames_hello_then_audio_ok() {
        ...
        feed(
            &mut accum,
            &encode(&inbound_hello(16_000, 16, 1, crate::wire::Codec::S16Le)),
        );
        let n = consume_with_state(&mut accum, &mut sink, &mut state)
    ...
    fn drain_inbound_retries_held_frame_without_new_bytes() {
        ...
        let hello = encode(&inbound_hello(16_000, 16, 1, crate::wire::Codec::S16Le));
    ...
    fn drain_oversize_pcm_is_error() {
        let max_pcm = crate::wire::MAX_AUDIO_PAYLOAD;
    ...
    // ── InboundObserver seam ───────────────────────────────────────────────

    /// Records every observer call so the platform seam's firing points can be asserted
    /// without a platform.
    struct RecordingObs {
        waypoints: Vec<(InboundWaypoint, u32)>,
        hellos: Vec<crate::wire::PlaybackFormat>,
    }
    impl RecordingObs {
        fn new() -> Self {
            RecordingObs {
    ...
    impl InboundObserver for RecordingObs {
        fn waypoint(&mut self, site: InboundWaypoint, frame: u32) {
            self.waypoints.push((site, frame));
        }
        fn hello_ok(&mut self, format: crate::wire::PlaybackFormat) {
            self.hellos.push(format);
        }
    }

    /// A conforming Hello fires `hello_ok` once with the *declared* format, and the
    /// post-Hello frame waypoints fire at frame 1 then every tenth accepted frame — the
    /// cadence the ESP heap sampling depends on.
    #[test]
    fn observer_sees_hello_and_periodic_waypoints() {
        let mut accum = FrameAccumulator::new();
        let mut sink = CapturingSink::new();
        let mut state = InboundConnectionState::new();
        let mut obs = RecordingObs::new();

        let declared = crate::wire::PlaybackFormat {
            sample_rate_hz: 16_000,
            bits_per_sample: 16,
            channels: 1,
            codec: crate::wire::Codec::S16Le,
        ...
        feed(
            ...
            &encode(&inbound_hello(
                declared.sample_rate_hz,
                declared.bits_per_sample,
                declared.channels,
                declared.codec,
            )),
        );
        consume_frames(&mut accum, &mut sink, &mut state, &mut obs)
            .expect("Hello consumes cleanly");
        assert_eq!(
            obs.hellos,
            vec![declared],
            "hello_ok fires once, carrying the accepted format"
        );
        assert!(
            obs.waypoints.is_empty(),
            "the handshake alone routes no audio frames"
        );

        // 21 audio frames → waypoints at 1, 10, 20.
        let audio = encode(&audio_frame(AUDIO_SAMPLES_PER_FRAME));
        for _ in 0..21 {
            feed(&mut accum, &audio);
            consume_frames(&mut accum, &mut sink, &mut state, &mut obs).expect("audio routes");
        }
        assert_eq!(
            obs.waypoints,
            vec![
                (InboundWaypoint::Periodic, 1),
                (InboundWaypoint::Periodic, 10),
                (InboundWaypoint::Periodic, 20)
            ],
            "waypoints fire at the first frame and every tenth thereafter"
        );
        assert_eq!(obs.hellos.len(), 1, "hello_ok does not re-fire mid-stream");
    }

    /// A rejected Hello fires nothing: the observer's window opens only on an accepted
    /// stream, so a mismatch must not look like a live inbound connection.
    #[test]
    fn observer_silent_on_rejected_hello() {
        let mut accum = FrameAccumulator::new();
        let mut sink = CapturingSink::new();
        let mut state = InboundConnectionState::new();
        let mut obs = RecordingObs::new();
        ...
        feed(
            &mut accum,
            &encode(&inbound_hello(48_000, 16, 1, crate::wire::Codec::S16Le)),
        );
        consume_frames(&mut accum, &mut sink, &mut state, &mut obs)
            .expect_err("a format-mismatch Hello drops the connection");
        assert!(obs.hellos.is_empty(), "no hello_ok on a rejected format");
        assert!(
            obs.waypoints.is_empty(),
            "no waypoints on a rejected format"
        );
        assert!(
            state.seen_hello(),
            "a format-mismatch Hello still marks the handshake seen — the flag is set \
             before the format check, so the teardown sample is taken for this connection"
        );
    }

    /// The teardown sample is gated on the handshake: a connection that never decoded a
    /// Hello records nothing, one that did records exactly one `Exit` carrying its
    /// accepted-frame count.
    #[test]
    fn exit_waypoint_fires_only_past_hello() {
        let mut obs = RecordingObs::new();
        let mut state = InboundConnectionState::new();
        note_inbound_exit(&mut obs, &state);
        assert!(
            obs.waypoints.is_empty(),
            "pre-Hello teardown takes no sample"
        );
        ...
        consume_frames(&mut accum, &mut sink, &mut state, &mut obs)
            .expect("Hello + 3 audio frames");
        obs.waypoints.clear();

        note_inbound_exit(&mut obs, &state);
        assert_eq!(
            obs.waypoints,
            vec![(InboundWaypoint::Exit, 3)],
            "post-Hello teardown takes one sample carrying the frame count"
        );
    }

    /// The waypoint tokens are the trailing word in the `heap wp inbound <token>` log
    /// line operators grep, so they are a pinned external contract, not free text.
    #[test]
    fn waypoint_tokens_are_stable() {
        assert_eq!(InboundWaypoint::Periodic.as_str(), "periodic");
        assert_eq!(InboundWaypoint::Exit.as_str(), "exit");
    }

    /// `seen_hello` / `inbound_frames` expose the state a teardown-path instrument needs
    /// to decide whether the connection ever entered the post-Hello audio window.
    #[test]
    fn state_accessors_track_handshake_and_frame_count() {
        let mut accum = FrameAccumulator::new();
        let mut sink = CapturingSink::new();
        let mut state = InboundConnectionState::new();

        assert!(!state.seen_hello(), "fresh state has not handshaken");
        assert_eq!(state.inbound_frames(), 0, "fresh state has routed nothing");
        ...
        feed(&mut accum, &encode(&audio_frame(AUDIO_SAMPLES_PER_FRAME)));
        feed(&mut accum, &encode(&audio_frame(AUDIO_SAMPLES_PER_FRAME)));
        consume_with_state(&mut accum, &mut sink, &mut state).expect("Hello + 2 audio frames");
        assert!(
            state.seen_hello(),
            "handshake is visible through the accessor"
        );
        assert_eq!(state.inbound_frames(), 2, "both audio frames counted");

        state.reset();
        assert!(!state.seen_hello(), "reset clears the handshake");
        assert_eq!(state.inbound_frames(), 0, "reset clears the frame count");
    }
}

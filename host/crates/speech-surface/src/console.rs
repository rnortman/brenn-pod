...
fn narrate_model_stats(fields: &Value) -> String {
    ...
    // No scored chunks — or a count that is absent or not a number — must not
    // be rendered as a distribution: "the model ran and could not score" must
    // not read as "it scored, values unknown".
    let count = match chunks {
        Some(n) if n > 0 => n.to_string(),
        Some(_) => return format!("{model} p x0{unscored} ({cause}) @{at}"),
        None => return format!("{model} p x?{unscored} ({cause}) @{at}"),
    };
...
mod tests {
    ...
    /// A line whose chunk count is missing or mistyped degrades to `x?` rather
    /// than to `x0`: "the model scored nothing" is a claim, and a line that
    /// never said how many chunks it covered is not entitled to make it. The
    /// distribution stays off the line for the same reason.
    #[test]
    fn model_stats_without_a_chunk_count_degrade_to_question_marks() {
        for chunks in [Value::Null, json!("many")] {
            let mut r = Renderer::new(false);
            let mut fields = json!({
                "pod": "pod-fbe2f8",
                "epoch": 1,
            ...
            if chunks.is_null() {
                fields.as_object_mut().unwrap().remove("chunks");
            } else {
                fields["chunks"] = chunks.clone();
            }
            let line = r.render(0, "model_stats", &fields).unwrap();
            assert!(
                line.contains("[pod-fbe2f8] oww p x? +8 unscored (segment_close) @10240"),
                "chunks {chunks}: {line}"
            );
            assert!(
                !line.contains("min"),
                "an uncounted line reports no distribution: {line}"
            );
        }
    }
